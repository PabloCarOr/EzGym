...
class RegistrosController {
    ...
    static async indexGet(req, res) {
        let data = await RegistrosModel.consultar();
        res.send(data);
    ...
    static async itemGet(req, res) {
        ...
        let data = await RegistrosModel.consultarPorId(id);
        if (data.length == 0) {
    ...
    static async indexPost(req, res) {
        try {
            ...
            const insertedId = await RegistrosModel.insertar(newData);

            res.status(201)
                .header('Location', `/registros/${insertedId}`)
    ...
    static async itemPatch(req, res) {
        try {
            ...
            const result = await RegistrosModel.actualizar(id, updatedFields);
